...
> 状态：草案 ｜ 更新日期：2026-08-28

> **当前运行态（2026-08-28）**：本地开发机无 Docker，Kafka / Elasticsearch / MinIO 暂不可用。已按接口抽象降级运行：转码队列 → DB 任务表、跨模块事件/扇出 → 进程内直调与 DB 轮询、对象存储 → 本地磁盘 Storage、搜索 → LIKE 模糊匹配（详见[进度管理 §3 阻塞与风险](/project/progress)）。部署环境就绪后按本文目标架构无缝切回，本文其余内容均按目标态描述。
