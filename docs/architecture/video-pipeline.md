...
> **当前运行态（2026-08-28）**：转码任务队列暂以 **DB 任务表**（video_transcode_job，Worker 轮询认领）替代 Kafka topic 实现，接口不变；其余（FFmpeg 多档 HLS / 抽帧封面 / 状态机）均已按本流水线落地。Kafka 就绪后切回，见[进度管理 §3](/project/progress)。
