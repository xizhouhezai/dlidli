...
- [ ] M1-VID-09 packages/player：hls.js 内核封装（清晰度/倍速/进度/快捷键）（播放页已内联 hls.js + 清晰度切换，抽包与弹幕引擎待做）
...
- [ ] M1-ADM-06 后台前端：用户管理页、分区管理页（用户管理页已完成随 ADM-03；敏感词库管理页随 ADM-04；分区管理页待做）
...
- [ ] M2-GRW-01 后端：经验/等级/权益规则引擎
- [ ] M2-GRW-02 后端：弹幕彩色/权限按等级控制
- [ ] M2-GRW-03 Web：等级展示、经验进度、硬币明细页
