...
- [ ] M2-DM-01 顶部/底部/彩色弹幕 + 弹幕列表面板
- [ ] M2-DM-02 用户屏蔽设置（关键词/用户/密度）
- [ ] M2-DM-03 WebSocket 实时弹幕（接 comet）
- [ ] M2-DM-04 弹幕举报 + 后台处理队列
...
- [ ] M2-AUD-01 图像/文本内容安全 API 接入（稿件/评论/弹幕/资料）
- [ ] M2-AUD-02 机审分级策略（低风险抽检/高风险全审）
- [ ] M2-AUD-03 举报体系（全对象类型 + 后台处理）
- [ ] M2-AUD-04 青少年模式（时长/内容池/入口弹窗）
