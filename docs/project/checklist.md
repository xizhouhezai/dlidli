...
- [ ] M2-SYS-01 后端+前端：审计日志中心（查询/筛选/导出）
- [ ] M2-SYS-02 系统配置项集中管理（键值 + 热更新）+ 数据字典维护
...
- [ ] M3-REC-01 行为日志采集管道（曝光/点击/播放/互动 → ClickHouse）
- [ ] M3-REC-02 热度榜计算任务（全站/分区，定时）
- [ ] M3-REC-03 ItemCF 离线计算 + 相似视频存储
- [ ] M3-REC-04 推荐服务（召回合并/规则粗排/多样性打散/已看过滤）
- [ ] M3-REC-05 首页信息流接入 + 负反馈（不感兴趣）
- [ ] M3-REC-06 新稿冷启动扶持池
- [ ] M3-REC-07 关闭个性化推荐开关（合规）
