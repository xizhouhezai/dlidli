...
- [ ] M2-DM-01 发送端 UI：输入框工具条（滚动/顶部/底部模式选择 + 8 预设色板，非白/顶底 Lv3 解锁置灰提示）；重复内容 30s 去重（后端）；弹幕列表面板（独立弹窗：时间排序全量列表分页 + 点击跳转进度 + 复制/举报/屏蔽发送者）
- [ ] M2-DM-02 屏蔽与展示设置：danmaku_block 表（服务端账号级：关键词 ≤200 + 用户按 UID 哈希）+ CRUD 接口 + 渲染前过滤（拉取/实时双链路）+ 设置页/成长中心屏蔽管理入口；弹幕设置面板（不透明度/字号缩放/显示区域 1/4·半屏·全屏/滚动速度/同屏密度）
- [ ] M2-DM-03 WebSocket 实时弹幕：/api/v1/ws/danmaku 视频房间广播（发送成功即时推送同视频在线连接）+ 断线自动重连 + WS 不可用回退 HTTP 分段拉取；单视频在线连接数上限（超出回退拉取）
- [ ] M2-DM-04 弹幕举报入口：前端入口（列表面板/悬停操作）；后端与后台处理队列已随 M2-AUD-03 完成（弹幕类型 + admin 删除/处罚）
