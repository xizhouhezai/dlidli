...
| 里程碑 | 计划窗口 | 状态 | 进度 | 备注 |
| --- | --- | --- | :-: | --- |
| M0 基建 | W1-W4 | ✅ 已完成 | 100% | staging 部署（ENG-13）收口，M0 全部 13 项完成 |
| M1 MVP 内测 | W5-W12 | 🟢 进行中 | 100% | player 内核封装（VID-09），M1 功能全部完成 |
| M2 V1.0 公测 | W13-W24 | ✅ 已完成 | 100% | SYS-01/02 收口，M2 全部 40 项完成 |
| M3 V2.0 增长 | W25-W48 | 🟢 进行中 | 79% | 私信 IM + 多P/合集 + A/B 实验 + 稿件管理 + 数据大盘 + 推荐系统 + 创作者中心 |
...
- [x] 私信 IM（M3-IM-01~03，PRD MSG-10~13）：0026 迁移 conversation/private_message；im 模块（发送：禁言+机审+未互关每日1条/互关不限+会话 upsert；会话列表含未读；消息分页读取即已读；总未读）；im hub 按 uid 分房间 WS 实时推送（query token）；/messages 页（会话列表+聊天窗气泡/图片/500字）+ 头部私信图标未读红点 + 空间页发私信；E2E 实测限制/互关放行/未读已读/敏感词/WS 实时/双向气泡全通

### 进行中

- [x] staging 部署（M0-ENG-13）：configs/staging.yaml（独立端口 8100/库 dlidli_staging/Redis db1/自动过审）+ deploy/staging.ps1（构建→迁移→启动→HelloWorld 验证）；修复迁移历史冲突（0001/0005 重复 relation、0022 与收藏夹 collection 冲突）；实测全流程通过且与 dev 完全隔离

### 下阶段计划

- [x] 私信拉黑/举报（MSG-12）：0027 迁移 user_block；relation Block/BlockStatus（POST /space/:id/block + block-status）；im Send 拉黑拦截（对方拉黑我→拒绝）；会话头部拉黑/举报按钮 + 被拉黑方输入禁用警告条 + 举报会话（target_type=5）；实测拉黑拦截/取消恢复/举报提交全通

### 下阶段计划

- [x] 列表数据导出（SYS-06）：admin 用户/稿件/举报三列表导出 CSV（复用筛选、UTF-8 BOM、上限 10000）；前端三页'导出 CSV'按钮；实测行数与页面一致、Excel 可直接打开

### 下阶段计划

- [ ] M3 架构演进：微服务拆分（M3-ENG-01~04，本地降级：单服务按模块内聚，分表/链路追踪待规模化）
- [ ] M1/M2 遗留：k6 压测（REL-01）、邀请码（REL-04）
