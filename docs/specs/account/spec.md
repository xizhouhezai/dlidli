...
> - 2026-08-28 补录 ACC-43 手机号加密存储（PII 合规）；将 §2.1 手机号注册/登录的标识存储由明文改为"密文 + 确定性哈希"两阶段迁移（详见 §2.4 下方迁移说明）
> - 2026-08-28 实现 ACC-02 邮箱注册/激活：user_auth 新增 activated 列（0029 迁移），激活 token 存 Redis 24h，激活邮件为 mock（dev 返回 debug 激活链接）
> - 2026-08-28 补录 ACC-44 内测邀请码注册（M1-REL-04）：invite_code 表 + 注册开关，一次性邀请码校验
...
| ID | 需求 | 验收标准（EARS） | 优先级 |
| --- | --- | --- | --- |
| ACC-01 | 手机号注册/登录 | **WHEN** 用户提交有效手机号与正确短信验证码 **THE SYSTEM SHALL** 为已注册手机号直接登录、为未注册手机号自动注册（默认昵称 dli_xxxx）并颁发登录凭证（验证码 60s 冷却、10 分钟有效）。手机号标识须按 ACC-43 密文存储 | P0 |
| ACC-02 | 邮箱注册/登录 | **WHEN** 用户以邮箱 + 密码注册 **THE SYSTEM SHALL** 创建待激活账号并发送激活邮件，**WHERE** 账号未激活 **THE SYSTEM SHALL** 拒绝登录 | P0 |
| ACC-44 | 内测邀请码注册（M1-REL-04 补录） | **WHERE** 内测邀请开关开启 **THE SYSTEM SHALL** 要求注册（短信自动注册/邮箱注册）携带未被使用且未过期的邀请码；**IF** 邀请码无效/已使用/已过期 **THEN THE SYSTEM SHALL** 拒绝注册并提示；**WHEN** 注册成功 **THE SYSTEM SHALL** 将邀请码标记为已使用（一次性） | P1 |
...
| ACC-43 | 手机号加密存储（PII 合规补录） | **THE SYSTEM SHALL** 对手机号标识以可逆加密（AES-GCM）密文存储，并另存确定性哈希（SHA-256）用于精确查重与查询；**WHEN** 管理员后台查看用户手机号 **THE SYSTEM SHALL** 解密展示；**WHERE** 未迁移的存量明文账号 **THE SYSTEM SHALL** 在登录/注册时按触发式惰性迁移加密 | P0 |

> **迁移说明（两阶段，禁止破坏性 DDL）**
> 1. 新增 `identifier_hash` 列；保留 `identifier`（待加密）与旧索引。哈希唯一性先由应用层 `FindAuth` 先查后插（事务）保证（存量行 hash 为空会碰撞，故第 1 步不建 hash 唯一索引）。
> 2. 新写入走"密文 + 确定性哈希"；存量明文账号在登录/注册时逐条惰性回填加密；管理端解密读取。
> 3. 全量回填完成并校验后，另发后续 release 补建 hash 唯一索引并删除明文 `identifier` 列与旧索引（不在本变更内，避免破坏性 DDL）。
