...
// Hub 通知实时推送中心（M2-MSG-02 comet）：按用户 ID 分房间，在线即推。
// 与 im.Hub 同构（独立包内实现，避免跨模块耦合）；前端由 60s 轮询升级为实时。
type Hub struct {
	mu       sync.RWMutex
...
// wsNotify 推送帧：type=notify，data 为通知条目（与列表接口同构，前端可直接插入列表头）。
type wsNotify struct {
	Type string `json:"type"`
	Data Item   `json:"data"`
}

// NewHub 构建通知 Hub（Origin 白名单防跨站 WS 劫持）。
...
// Push 向用户推送一条通知（在线时；无连接 no-op）。
// 迭代期间持读锁：send 为非阻塞投递不会死锁，且与 remove 的写锁互斥，
// 避免并发迭代/写 map 与 send on closed channel。
func (h *Hub) Push(uid int64, item Item) {
	if h == nil {
		return
	}
	msg, err := json.Marshal(wsNotify{Type: "notify", Data: item})
	if err != nil {
		return
	}
	h.mu.RLock()
	...
	for c := range r.clients {
		...
		case c.send <- msg:
		default:
			h.log.Warn("通知推送丢弃（慢消费者）", zap.Int64("uid", uid))
		}
	}
}
...
// readLoop 客户端只读（通知为服务端单向推送）；异常/关闭时移除。
