...
const { list: admins, total, loading, page, pageSize, load, onPageChange } = usePagedList<AdminAccount>(
  (p, size) => adminApi.admin.admins(p, size),
)
onMounted(async () => {
...
function openEdit(a: AdminAccount) {
  editing.value = a
  form.value = { username: a.username, nickname: a.nickname, password: '', roleIds: [...a.role_ids] }
  dialogVisible.value = true
}
  ...
  const ok = await run(async () => {
    if (editing.value) {
      await adminApi.admin.updateAdmin(editing.value.id, {
        username: form.value.username, nickname: form.value.nickname, role_ids: form.value.roleIds,
      })
    } else {
      await adminApi.admin.createAdmin({
        username: form.value.username, nickname: form.value.nickname,
        password: form.value.password, role_ids: form.value.roleIds,
      })
    }
...
async function toggle(a: AdminAccount) {
  ...
  const ok = await run(() => adminApi.admin.toggleAdmin(a.id, next), { success: next === 0 ? '已启用' : '已停用' })
  if (ok) load()
}
