...
const {
  ...
  search,
  onPageChange,
} = usePagedList<ReportItem>((p, size) =>
  adminApi.admin.reports({ status: status.value, page: p, page_size: size }),
)
...
async function handleDelete(r: ReportItem) {
  await ElMessageBox.confirm(
    `确定删除该${TARGET_TYPE_LABEL[r.target_type]}内容吗？删除后作者与游客均不可见。`,
    '删除内容',
    { type: 'warning' },
  )
  const ok = await run(
...
async function handlePunish(r: ReportItem) {
  const { value: punish } = await ElMessageBox.prompt(
    '处罚类型：mute 禁言 / ban 封禁（0=永久）',
    '删除并处罚作者',
    {
      inputValue: 'mute',
  ...
  const { value: daysStr } = await ElMessageBox.prompt(
    punish === 'mute' ? '禁言天数（1/3/7/30）' : '封禁天数（0=永久，或 7/30/90）',
    '处罚时长',
    {
...
async function exportCsv() {
  await run(
    () =>
      adminApi.http
        .download('/api/v1/admin/reports/export', {
          params: { status: status.value },
          fallbackName: `reports-${Date.now()}.csv`,
        })
        .then(saveBlob),
...
<template>
  <div>
    <PageHead title="举报处理" :sub="`共 ${total} 条举报`" />
    ...
    <div class="page-card">
      ...
      <div class="flex gap-2 mb-4">
        ...
        <el-button v-perm="'report:view'" class="pink-btn ml-auto" @click="exportCsv">
          导出 CSV
        </el-button>
      </div>
      ...
      <el-table v-loading="loading" :data="reports" stripe>
        <el-table-column label="对象" min-width="200">
          <template #default="{ row }">
            <div class="flex items-center gap-2">
              ...
              <span class="truncate" :title="row.target_desc">{{ truncate(row.target_desc) }}</span>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="补充说明" min-width="140">
          <template #default="{ row }">
            <span v-if="row.reason" class="truncate block" :title="row.reason">{{
              truncate(row.reason, 30)
            }}</span>
            <span v-else>—</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <template v-if="row.status === 0">
              <el-button v-perm="'report:handle'" link type="primary" @click="handleDelete(row)">
                删除
              </el-button>
              <el-button v-perm="'report:handle'" link type="danger" @click="handlePunish(row)">
                删除并处罚
              </el-button>
              <el-button v-perm="'report:handle'" link @click="handleIgnore(row)"> 忽略 </el-button>
            </template>
            <span v-else>—</span>
          </template>
        </el-table-column>
